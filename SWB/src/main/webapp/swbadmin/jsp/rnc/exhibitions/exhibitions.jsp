...
<%@page import="mx.gob.cultura.portal.response.DigitalObject, mx.gob.cultura.portal.response.Entry, mx.gob.cultura.portal.response.Identifier, org.semanticwb.portal.api.SWBParamRequest"%>
<%@ page import="java.util.List, org.semanticwb.model.Resource" %>
<%
	Resource base = (Resource)request.getAttribute("base");
    SWBParamRequest paramRequest = (SWBParamRequest)request.getAttribute("paramRequest");
    List<Entry> itemsList = (List<Entry>)request.getAttribute("references");
%>
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 car-nav2">
                <div class="owl-nav col">
                    <h3 class="oswB blanco"><%=base.getAttribute("title","")%></h3>
                            ...
                            <div class="card">
                                ...
                                <%
                                    for (DigitalObject digObj : item.getDigitalobject()) {
                                        if (digObj.getMediatype().getMime().startsWith(base.getAttribute("type","image")) && !digObj.getUrl().isEmpty()) {
                                %>
                                ...
                                <div class="card-footer">
                                    <small class="text-muted">
                                        ...
                                        <div><a class="rojo" href="/swb/<%=paramRequest.getWebPage().getWebSiteId()%>/detalle?id=<%=item.getId()%>">Ver m&aacute;s +</a></div>
                                    </small>
                                </div>
                            </div>
                        </div>
