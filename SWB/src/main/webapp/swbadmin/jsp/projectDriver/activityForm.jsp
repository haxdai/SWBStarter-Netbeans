...
<%@ page import="org.semanticwb.portal.resources.projectdriver.base.ActivityBase" %>
<%
    ...
    %>
      ...
      <%
        ...
        <form id="<%=act.getURI()+"/form"%>" name="<%=act.getURI()+"/form"%>" class="edit" action="<%=url.toString()%>" method="post">
            ...
            <table class="detail">
              <tbody>
              <tr><td width="200px"><%=mgr.renderLabel(request, ActivityBase.swbproy_critical,mgr.MODE_EDIT)%></td>
                  <td><%
                  if(act.getStatus().equals("canceled")||act.getStatus().equals("ended")){String checked="";
                      if(act.isCritical()){checked="checked=\"checked\"";}
                      out.println("<input type=\"checkbox\" id=\""+ ActivityBase.swbproy_critical.getName()+"\" name=\""+ ActivityBase.swbproy_critical.getName()+"\""+checked+"\" disabled=\"disabled\">");
                  }else
                       out.println(mgr.renderElement(request, ActivityBase.swbproy_critical,mgr.MODE_EDIT));
    %>             </td>
              </tr>
              <tr><td width="200px"><%=mgr.renderLabel(request, ActivityBase.swbproy_actType, mgr.MODE_EDIT)%></td>
                  <td><%
                  if(act.getStatus().equals("canceled")||act.getStatus().equals("ended")){
                     String value=act.getActType();
                     if(value==null)value="";
                     out.println("<input id=\""+ ActivityBase.swbproy_actType.getName()+"\" name=\""+ ActivityBase.swbproy_actType.getName()+"\" value=\""+value+"\" style=\"width:300px\" disabled=\"disabled\">");
                  }else
                   out.println(mgr.renderElement(request, ActivityBase.swbproy_actType,mgr.MODE_EDIT));
    %>             </td>
              </tr>
              <tr><td width="200px"><%=mgr.renderLabel(request, ActivityBase.swbproy_status, mgr.MODE_EDIT)%></td><td><select name="<%=Activity.swbproy_status.getName()%>" <%if(checkPrede){%>disabled<%}%>>
              ...
              <tr><td width="200px"><%=mgr.renderLabel(request, ActivityBase.swbproy_hasPredecessor, mgr.MODE_EDIT)%></td>
                  <td><%
                   ArrayList vals1=tool.getActsPrede(act,user);
                   ArrayList listAll=tool.getActsForPrec(act, parent);
                   it = listAll.iterator();
                   %><select name="<%=ActivityBase.swbproy_hasPredecessor.getName()%>" multiple="true" size="4" style="width:300px;" <%if(act.getStatus().equals("canceled")||act.getStatus().equals("ended")){%>disabled<%}%>><%
              ...
              <tr><td width="200px"><%=mgr.renderLabel(request, ActivityBase.swbproy_responsible, mgr.MODE_EDIT)%></td>
                  <td><%
                  %><select name="<%=ActivityBase.swbproy_responsible.getName()%>" disabled><%
              ...
              <tr><td width="200px"><%=mgr.renderLabel(request, ActivityBase.swbproy_hasParticipants, mgr.MODE_EDIT)%></td>
                  <td><%
                  ...
                  %><select name="<%=ActivityBase.swbproy_hasParticipants.getName()%>" multiple="true" size="4" style="width:300px;" <%
                    if(act.getStatus().equals("canceled")||act.getStatus().equals("ended")){
                            %>disabled<%
                    }%>><%if(it.hasNext())%><option value=""><%
                    while (it.hasNext()) {
        ...
        <fieldset><legend><%=paramRequest.getLocaleString("labelActivityDetail")%></legend>
          ...
          <table  class="detail">
            <tbody>
               ...
               <tr><td><%=mgr.renderLabel(request, ActivityBase.swbproy_actType, mgr.MODE_VIEW)%></td><%
                  if(act.getActType().equals("")){%>
                  <td><%=paramRequest.getLocaleString("msgUnassigned")%></td>
                  <%}else{%>
                  <td><%=mgr.renderElement(request, ActivityBase.swbproy_actType,mgr.MODE_VIEW)%></td>
                  <%}
                   %>
               </tr>
               <tr><td><%=mgr.renderLabel(request, ActivityBase.swbproy_critical,mgr.MODE_VIEW)%></td>
                   <td><%=mgr.renderElement(request, ActivityBase.swbproy_critical,mgr.MODE_VIEW)%></td>
               </tr>
               <tr><td><%=mgr.renderLabel(request, ActivityBase.swbproy_hasPredecessor, mgr.MODE_VIEW)%></td>
                   <td>
                    <%GenericIterator <Activity> it1 = act.listPredecessors();
                    if(it1.hasNext()){%>
                     <select name="<%=ActivityBase.swbproy_hasPredecessor.getName()%>" size="3" multiple="true" style="width:200px;">
                      <%while(it1.hasNext()){
               ...
               <tr><td><%=mgr.renderLabel(request, ActivityBase.swbproy_status, mgr.MODE_VIEW)%></td><%
               ...
               <tr><td><%=mgr.renderLabel(request, ActivityBase.swbproy_responsible, mgr.MODE_VIEW)%></td>
                   <td>
                     <%if(act.getResponsible()!=null){%>
               ...
               <tr><td><%=mgr.renderLabel(request, ActivityBase.swbproy_hasParticipants, mgr.MODE_VIEW)%></td>
                   <td>
                    <%GenericIterator<User> it2 = act.listParticipantses();
                    if(it2.hasNext()){%>
                     <select name="<%=ActivityBase.swbproy_hasParticipants.getName()%>" size="2" multiple="true" style="width:200px;">
                      <%while(it2.hasNext()){
                        User us=it2.next();%>
